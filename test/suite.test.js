...
describe('WebSocket – /media-stream', () => {
    test('unsigned upgrades are refused', async () => {
        await new Promise((resolve, reject) => {
            const ws = new WebSocket(`${WS_BASE}/media-stream`);
            const timer = setTimeout(() => { ws.terminate(); reject(new Error('Upgrade did not finish')); }, 5000);
            ws.on('open', () => { clearTimeout(timer); ws.terminate(); reject(new Error('Unsigned stream accepted')); });
            ws.on('unexpected-response', (_request, response) => {
                clearTimeout(timer);
                response.resume();
                ws.terminate();
                try { assert.ok([403, 503].includes(response.statusCode)); resolve(); } catch (error) { reject(error); }
            });
            ws.on('error', () => {});
        });
    });
