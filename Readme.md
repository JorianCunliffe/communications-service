...
- Before publishing migrations `019`-`021` on Replit, validate that the generated database comparison contains no table truncations, dropped records, or destructive warnings. The compatibility default in migration `021` lets Replit add `normalized_value` without clearing existing identities; the production migration runner must then record migrations `019`-`021`, backfill real values, and install the trigger and SQL functions before the API starts.
