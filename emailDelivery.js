...
import { assertEmailSendAllowed, readEmailPolicy } from './emailPolicy.js';
...
export async function sendEmailWithProvider({ connection, request, idempotencyKey, database = getDatabase() }) {
    const policy = await readEmailPolicy(database, connection?.tenant_id);
    assertEmailSendAllowed(connection?.tenant_id, policy.mode);
